...
function FindRide() {
  ...
  const location = (query, setSuggestion) => {
    clearTimeout(timerRef.current);

    timerRef.current = setTimeout(async () => {
      try {
        const res = await fetch(
          `https://api.locationiq.com/v1/search?key=${locationToken}&q=${query}&format=json&countrycodes=in`
        );
        const data = await res.json();
        setSuggestion(data || []);
      } catch (err) {
      ...
      <div className="search-box">
        ...
        <div className="input-group">
          <input placeholder="From" value={from} onChange={(e) => {
            ...
            setShowFrom(true);
            location(e.target.value, setFromSuggeation);
          }}
            onKeyDown={(e) => {
        ...
        <div className="input-group">
          <input placeholder="To" value={to} onChange={(e) => {
            ...
            setShowTo(true);
            location(e.target.value, setToSuggeation);
          }}
            onKeyDown={(e) => {
