...
exports.getRoutes = async (req, res) => {
    try {
        ...
        const apiKey = pk.c3888526c247eef962f5eaa5f35860fb;

        const ridesWithRoutes = await Promise.all(
            rides.map(async (ride) => {
                if (!ride.pickup || !ride.drop) return ride;

                const start = `${ride.pickup.longitude},${ride.pickup.latitude}`;
                const end = `${ride.drop.longitude},${ride.drop.latitude}`;

                const url = `https://us1.locationiq.com/v1/directions/driving/${start};${end}?key=${apiKey}&geometries=geojson`;
        ...
        res.status(200).json({
            count: ridesWithRoutes.length,
            rides: ridesWithRoutes
        });
...
exports.getPassengers = async (req, res) => {
    try {
        const { id } = req.params;
        const bookings = await Booking.find({ ride: id, status: "confirmed" }).populate("passenger");
        res.status(200).json({
